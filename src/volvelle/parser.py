...
from volvelle.subcommands import (
    clipboard,
    emoji,
    install,
    record,
...
from volvelle.utils.dots.manifest import Manifest
from volvelle.utils.dots.packages import AUR_HELPERS
from volvelle.utils.dots.source import DotsSource
from volvelle.utils.io import warn
from volvelle.utils.paths import wallpapers_dir
from volvelle.utils.scheme import get_scheme_names, scheme_variants
from volvelle.utils.wallpaper import get_wallpaper


def parse_args() -> tuple[argparse.ArgumentParser, argparse.Namespace]:
    parser = argparse.ArgumentParser(
        prog="volvelle", description="Command-line interface for Volvelle"
    )
    parser.add_argument(
        "-v",
        "--version",
    ...
    shell_parser = command_parser.add_parser(
        "shell", help="start or message the shell"
    )
    ...
    shell_parser.add_argument(
        ...
        "--restart",
        action="store_true",
        help="kill and restart the shell",
    )
    shell_parser.add_argument(
        "-s",
        "--show",
        action="store_true",
        help="print all shell IPC commands",
    )
    shell_parser.add_argument(
    ...
    shell_parser.add_argument(
        "--log-rules", metavar="RULES", help="log rules to apply"
    )
    ...
    scheme_parser = command_parser.add_parser(
        "scheme", help="manage the colour scheme"
    )
    ...
    get_parser = scheme_command_parser.add_parser(
        "get", help="get scheme properties"
    )
    ...
    get_parser.add_argument(
        ...
        "--name",
        action="store_true",
        help="print the current scheme name",
    )
    get_parser.add_argument(
        "-f",
        "--flavour",
        action="store_true",
        help="print the current scheme flavour",
    )
    get_parser.add_argument(
        "-m",
        "--mode",
        action="store_true",
        help="print the current scheme mode",
    )
    get_parser.add_argument(
        "-v",
        "--variant",
        action="store_true",
        help="print the current scheme variant",
    )

    set_parser = scheme_command_parser.add_parser(
        "set", help="set the current scheme"
    )
    ...
    set_parser.add_argument(
        ...
        "--random",
        action="store_true",
        help="switch to a random scheme",
    )
    set_parser.add_argument(
        "-n",
        "--name",
    ...
    set_parser.add_argument(
        "-f", "--flavour", help="the flavour to switch to"
    )
    set_parser.add_argument(
        "-m",
        "--mode",
    ...
    set_parser.add_argument(
        "-v",
        "--variant",
        choices=scheme_variants,
        help="the variant to switch to",
    )
    ...
    record_parser = command_parser.add_parser(
        "record", help="start a screen recording"
    )
    ...
    record_parser.add_argument(
        ...
        "--pause",
        action="store_true",
        help="pause/resume the recording",
    )
    record_parser.add_argument(
        "-c",
        "--clipboard",
        action="store_true",
        help="copy recording path to clipboard",
    )
    ...
    clipboard_parser.add_argument(
        ...
        "--delete",
        action="store_true",
        help="delete from clipboard history",
    )

    # Create parser for emoji-picker opts
    emoji_parser = command_parser.add_parser(
        "emoji", help="emoji/glyph utilities"
    )
    ...
    emoji_parser.add_argument(
        ...
        "--picker",
        action="store_true",
        help="open the emoji/glyph picker",
    )
    emoji_parser.add_argument(
        "-f",
        "--fetch",
        action="store_true",
        help="fetch emoji/glyph data from remote",
    )
    ...
    resizer_parser = command_parser.add_parser(
        "resizer", help="window resizer daemon"
    )
    ...
    resizer_parser.add_argument(
        "height", nargs="?", help="height to resize to"
    )
    resizer_parser.add_argument(
        "actions",
        nargs="?",
        help="comma-separated actions to apply (float,center,pip)",
    )

    # Create parser for install opts
    install_parser = command_parser.add_parser(
        "install",
        help="install the configured desktop files",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    install_parser.set_defaults(cls=install.Command)
    install_parser.add_argument(
        "--aur-helper", choices=AUR_HELPERS, help="the AUR helper to use"
    )
    install_parser.add_argument(
        "--enable-components",
        metavar="LIST",
    ...
    update_parser = command_parser.add_parser(
        "update", help="update the configured desktop files"
    )
...
def _components_epilog(manifest: Manifest) -> str:
    ...
    lines = [
        f"{b(34)}available components (for --enable-components / --disable-components):{reset}"
    ]
    for name, comp in manifest.components.items():
        lines.append(
            f"  {b(32)}{name:<{width}}{reset}\t{'(default)' if comp.default else '(off)'}"
        )
    return "\n".join(lines)
