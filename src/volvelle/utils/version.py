...
from volvelle.utils.dots.packages import ArchInstaller
from volvelle.utils.dots.source import DotsSource, SourceError
from volvelle.utils.dots.state import DotsState
from volvelle.utils.paths import config_dir

PKGS = ("volvelle-shell", "volvelle-cli", "quickshell")
INDENT = "    "
...
def fetch_git_metadata(
    repo_dir: Path, branch: str = "upstream/main"
) -> tuple[str, str] | None:
    try:
...
def print_packages() -> None:
    ...
    missing = [
        (pkg, "not installed") for pkg, result in installed if result is None
    ]
...
def print_dots_version() -> None:
    ...
    _rows(
        [
            ...
            (
                "AUR helper",
                state.aur_helper if shutil.which("pacman") else "not on Arch",
            ),
        ]
    )
...
def print_version() -> None:
    ...
    try:
        shell_ver = subprocess.check_output(
            ["/usr/lib/volvelle/version", "-s"], text=True
        ).strip()
        _header("Shell:")
    ...
    print()
    if shutil.which("qs"):
        qs_ver = subprocess.check_output(
            ["qs", "--version"], text=True
        ).strip()
        _header("Quickshell:")
    ...
    local_shell_dir = config_dir / "quickshell/volvelle"
        ...
        _rows(
            [
                (
                    ...
                    _commit_from_meta(upstream_metadata),
                ),
