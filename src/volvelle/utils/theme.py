...
from volvelle.utils.colour import get_dynamic_colours
from volvelle.utils.hypr import is_lua_config
from volvelle.utils.io import log_exception
...
from volvelle.utils.scheme import get_scheme
...
def apply_discord(scss: str) -> None:
    ...
    for client in "Equicord", "Vencord", "BetterDiscord", "equibop", "vesktop", "legcord":
        atomic_write(config_dir / client / "themes/volvelle.theme.css", conf)
...
def apply_pandora(colours: dict[str, str], mode: str) -> None:
    ...
    atomic_write(data_dir / "PandoraLauncher/themes/volvelle.json", template)


@log_exception
def apply_spicetify(colours: dict[str, str], mode: str) -> None:
    template = gen_replace(colours, templates_dir / f"spicetify-{mode}.ini")
    atomic_write(config_dir / "spicetify/Themes/volvelle/color.ini", template)
...
def apply_btop(colours: dict[str, str]) -> None:
    ...
    atomic_write(config_dir / "btop/themes/volvelle.theme", template)
...
def apply_qt(colours: dict[str, str], mode: str, icon_theme: str | None = None) -> None:
    ...
    atomic_write(config_dir / "qtengine/volvelle.colors", colours)
...
def apply_warp(colours: dict[str, str], mode: str) -> None:
    ...
    atomic_write(data_dir / "warp-terminal/themes/volvelle.yaml", template)
...
def apply_chromium(colours: dict[str, str]) -> None:
    ...
    for cmd, policy_dir in browsers:
        ...
        subprocess.run(
            ["sudo", "-n", "tee", str(policy_dir / "volvelle.json")],
...
def apply_zed(colours: dict[str, str], mode: str) -> None:
    theme_path = config_dir / "zed/themes/volvelle.json"
