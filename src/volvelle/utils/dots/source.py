...
from volvelle.utils.paths import dots_dir, get_config
...
class DotsSource:
    ...
    def ensure(self) -> None:
        ...
        self._run(
            "git", "clone", "--branch", self.branch, self.url, str(dots_dir)
        )
        DotsSource._fetched_source = True
    ...
    def blob_at(self, ref: str, relpath: str) -> bytes:
        ...
        if key not in self._blob_cache:
            self._blob_cache[key] = self._git_bytes(
                "show", f"{ref}:{relpath}"
            )
    ...
    def _git(self, *args: str) -> str:
        ...
        return self._run(
            "git", "-C", str(dots_dir), "-c", "core.quotePath=false", *args
        )
    ...
    def _run(self, *cmd: str) -> str:
        ...
        if result.returncode != 0:
            raise SourceError(
                result.stderr.strip() or f"{' '.join(cmd)} failed"
            )
        return result.stdout
