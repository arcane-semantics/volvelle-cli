...
from volvelle.utils.paths import c_cache_dir
...
class Command:
    ...
    def shell(self, *args: str) -> str:
        return subprocess.check_output(["qs", "-c", "volvelle", *args], text=True)

    def start_shell(self) -> None:
        args = ["qs", "-c", "volvelle", "-n"]
    ...
    def list_instances(self) -> list[dict]:
        proc = subprocess.run(["qs", "-c", "volvelle", "list", "-j"], check=False, capture_output=True, text=True)
    ...
    def stop_instances(self) -> None:
        ...
        for pid in pids:
            subprocess.run(["qs", "-c", "volvelle", "kill", "--pid", str(pid)], check=False, stdout=subprocess.DEVNULL)
