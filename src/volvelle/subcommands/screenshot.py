...
from volvelle.utils import hypr
from volvelle.utils.notify import notify
from volvelle.utils.paths import screenshots_cache_dir, screenshots_dir
...
class Command:
    ...
    def region(self) -> None:
        if self.args.region == "slurp":
            subprocess.run(
                ["qs", "-c", "volvelle", "ipc", "call", "picker", "openFreeze" if self.args.freeze else "open"]
            )
        else:
