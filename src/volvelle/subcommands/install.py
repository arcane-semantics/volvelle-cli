...
from volvelle.utils.dots.deployer import Deployer
from volvelle.utils.dots.manifest import (
    ComponentError,
    Manifest,
    ManifestError,
)
from volvelle.utils.dots.misc import build_local_packages, run_hooks
from volvelle.utils.dots.packages import (
    DEFAULT_AUR_HELPER,
    PackageError,
    PackageInstaller,
)
from volvelle.utils.dots.source import DotsSource, SourceError
from volvelle.utils.dots.state import DotsState
from volvelle.utils.io import (
    confirm,
    disable_input,
    fatal,
    info,
...
from volvelle.utils.paths import (
    config_backup_dir,
    config_dir,
)
...
class Command:
    ...
    def run(self) -> None:
        ...
        try:
            installer, packages, local_packages = self.install_packages(
                source, manifest
            )
    ...
    def print_greeting(self) -> None:
        print("\033[38;2;150;241;241m" + textwrap.dedent(r"""
                ╭─────────────────────────────────────────────────╮
                │                    Volvelle                     │
                │                                                 │
                ╰─────────────────────────────────────────────────╯
                """) + "\033[0m")  # Volvelle colour
        info("Welcome to the Volvelle desktop-file installer!")
        info("Here's a quick overview on what this command is going to do:")
        info("  - Install dependencies")
        info("  - Install config files")
        info(
            "The installer does NOT set up hardware/system level configs (e.g. drivers). Please do this yourself."
        )
        pause()
        print()
    ...
    def create_backup(self) -> None:
        if config_dir.exists():
            ...
            if config_backup_dir.exists():
                if not confirm(
                    "A backup already exists, overwrite?", default=False
                ):
    ...
    def install_packages(
        ...
        installer = PackageInstaller.get(
            self.args.aur_helper, self.args.noconfirm
        )
        ...
        if local_dirs:
            ...
            local_packages = build_local_packages(
                installer, source, local_dirs
            )
    ...
    def print_done(self) -> None:
        print()
        info("All done! Volvelle has been installed.")
        info("A few things to finish up:")
        info("  - A reboot is recommended for all changes to take effect")
        info(
            "  - Edit `~/.config/volvelle/hypr-vars.lua` to set default apps, keybinds and much more"
        )
        info(
            "  - Edit `~/.config/volvelle/hypr-user.lua` to set your monitor layout and other Hyprland configs"
        )
        info("  - Run `volvelle update` later to pull in the latest changes")
        info("Enjoy!")
